...
{
public:
    void Move(Player& playerObject, vector<Shield> shield);
    void checkEnemyCollision(Player& playerObject, vector<Shield> shield);
};
...
void missileEnemy::Move(Player& playerObject, vector<Shield> shield)
{
    y += 5;
    checkEnemyCollision(playerObject, shield);
}

void missileEnemy::checkEnemyCollision(Player& playerObject, vector<Shield> shield)
{
...
void standardEnemy::generatePath1(const int startingYLocation)
{
    ...
    for (float i = -50; i < 600; i++) { //assuming 100 fps (10ms sleep/frame)
...
{
    // Mapping some equation to the path
    for (float i = 600; i > -30; i--) { // 200 elements for a 4 second enemy on screen time, assuming 50 fps (25ms sleep/frame)
...
void standardEnemy::Move(Player& playerObject, MissileStandard missileStandard[nMissiles], MissileRapid missileRapid[nMissiles], MissileSpreadM missileSpreadM[nMissiles], MissileSpreadL missileSpreadL[nMissiles], MissileSpreadR missileSpreadR[nMissiles], vector<Shield> shield)
{
    if (state > 0) {
        ...
        for (int i = 0; i < nMissiles; i++)
        {
            ...
            if (isShot && (missileStandard[i].state == 1)) {
                state--;
        ...
        for (int i = 0; i < nMissiles; i++)
        {
            ...
            if (isShot && (missileRapid[i].state == 1)) {
                state--;
        ...
        for (int i = 0; i < nMissiles; i++)
        {
            ...
            if (isShot && (missileSpreadM[i].state == 1)) {
                state--;
        ...
        for (int i = 0; i < nMissiles; i++)
        {
            ...
            if (isShot && (missileSpreadL[i].state == 1)) {
                state--;
        ...
        for (int i = 0; i < nMissiles; i++)
        {
            ...
            if (isShot && (missileSpreadR[i].state == 1)) {
                state--;
